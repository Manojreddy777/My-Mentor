...
export default class taRoleUserClm extends React.Component {
    ...
    getDet(tnnt_id) {
      ...
      getDet_det(type) {
        RoleUserClmHelper.getAllActive()
         .then((data) => {
            if (type == "export") {
                  const TABLE_HEADER = {
                         ...
                         role_name: "Role Name",
                         clm_view: "Claim View",
                         clm_create: "Claim create",
                         clm_update: "Claim Update",
                         clm_export: "Claim Export",                              
                 };
                 const formattedData = [];
                 data.forEach((d, i) => {
                     formattedData.push({
                 ...
                 exportCSVFile(
                     TABLE_HEADER,
                     formattedData,
                     "Role User Claim Details" + moment().format("DDMMYYhhmmss")
                 );
             } else {
             this.setState({ array_taRoleUserClm: data });
             }
            ...
            {roleUserEditVisibility && (
                ...
                <div className={styles.wrapper}>                        
                    ...
                    <div className={styles.excelcontainer}>
                           <button
                             onClick={() => this.getDet_det("export")}
                             className={"button"}
                            >
                              Export
                          </button>
