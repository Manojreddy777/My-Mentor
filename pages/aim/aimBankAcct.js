...
import Component from "../../components/aim/aimBankAcct";
import BankAcctHelper from "../../helper/aim/aimBankAcct";
...
export default class AimBankAcct extends React.Component {
  ...
  getPageDet(filter) {
    const { com_id, tnnt_id, is_active, user_name  } = this.state;

    filter.com_id = com_id;
    filter.tnnt_id = global.config.tnnt_id;
    filter.is_active = is_active;
    filter.user_name= global.config.username,
    

    BankAcctHelper.getPageDet(filter)
      .then((data) => {
  ...
  getFilteredData(filter) {
    BankAcctHelper.getFilteredData(filter)
      .then((data) => {
        this.setState({
          arr_exportData: data,
          columnHeaders: {
                  ...
                  c.clm_export == 13 && (
                    <div className={styles.excelcontainer}>
                      <ExportExcel
                        ...
                        fileName={
                          "aimBankAcct_" + moment().format("DDMMYYhhmmss")
                        }
