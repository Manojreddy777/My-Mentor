...
import Component from "../../components/pfmMgmt/pfAudUserSessionDet";
import AudUserSessionDetHelper from "../../helper/pfmMgmt/pfAudUserSessionDet";
...
export default class PfAudUserSessionDet extends React.Component {
  ...
  getPageDet(filter) {
    ...
    AudUserSessionDetHelper.getPageDet(filter)
      .then((data) => {
  ...
  getFilteredData(filter) {
    AudUserSessionDetHelper.getFilteredData(filter)
      .then((data) => {
        this.setState({
          arr_exportData: data,
          columnHeaders: {
            ...
            // table_header1
                ...
                logoff_timestamp:"Logoff Timestamp",
                device:"Device",
...
{/* // table_column */}
                <td>{t.id}</td>
                <td>{t.user_name}</td>
                <td>{t.login_timestamp}</td>
                <td>{t.logoff_timestamp}</td>
                <td>{t.device}</td>
                  ...
                  c.clm_export == 13 && (
                    <div className={styles.excelcontainer}>
                      <ExportExcel
                        ...
                        fileName={"pfAudUserSessionDet_" + moment().format("DDMMYYhhmmss")}
