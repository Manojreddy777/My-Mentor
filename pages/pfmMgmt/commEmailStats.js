...
export default class EmailNotification extends React.Component {
  ...
  getPageDet() {
    ...
    PfNotifEmailHelper.getEmailCount(filter)
      .then((data) => {
  ...
  render() {
    const { arr_parentTable, err_message, pageNumber } = this.state;
    ...
    return (
      <div>
        <SideMenu tag="pfEmailStats">
          <Head title="Email Statistics" />
          <div className={styles.wrapper}>
            <table className={`table ${styles.table}`}>
              <thead>
                <tr>
                  <th>Total number of email sent</th>
                  <th>Delivered</th>
                  <th>Read</th>
                </tr>
              </thead>
